...
Global Const $unresolved_colour = "#F5F5F5"		; whitesmoke
Global Const $trivial_colour = "#D3D3D3"		; lightgray
Global Const $minor_colour = "#32CD32"			; limegreen
Global Const $major_colour = "#FFA500"			; orange
Global Const $critical_colour = "#FF0000"		; red
Global Const $blocker_colour = "#8B0000"		; darkred
...
;_DateTimeSplit($tt, $aMyDate, $aMyTime)
;$uu = StringFormat("%02i-%02i-%04i", $aMyDate[3], $aMyDate[2], $aMyDate[1])

;ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $uu = ' & $uu & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;_ArrayDisplay($aMyDate)
;Exit

;$yy = _DateTimeFormat($tt, 2)
...
Global $main_gui = GUICreate("JR - " & $app_name, 860, 600)
...
;_JiraLogin(GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input))
;_JiraGetVersions("RMS")
;$jira_json = '{"versions":' & $jira_json & "}"
;Local $decoded_json = Json_Decode($jira_json)
;ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $jira_json = ' & $jira_json & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;Local $name = Json_Get($decoded_json, '.versions[0].name')
...
;$issue = _JiraGetSearchResultBugChangeLogs("changelog", "project = " & GUICtrlRead($epic_key_input) & " AND issuetype = Bug")
;$jira_json = FileRead("D:\dwn\fred.txt")
;Local $decoded_json = Json_Decode($jira_json)
;Local $key = Json_Get($decoded_json, '.issues[1].key')
;ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $key = ' & $key & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;Local $created = Json_Get($decoded_json, '.issues[1].changelog.histories[0].created')
;ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $created = ' & $created & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;Exit
...
;While _DateDiff("D", $now_date, $date) < 0

;	$date = _DateAdd("D", 1, $date)
;	$sqlite_date = StringReplace($date, "/", "-")
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $sqlite_date = ' & $sqlite_date & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console

;	Local $num_opened_bugs, $num_resolved_bugs, $iRows, $iColumns

;	_SQLite_GetTable2d(-1, "select count(*) from BugStateDate where OpenDate <= '" & $sqlite_date & "'", $num_opened_bugs, $iRows, $iColumns)
;	_SQLite_GetTable2d(-1, "select count(*) from BugStateDate where ResolvedDate <> '' and ResolvedDate <= '" & $sqlite_date & "'", $num_resolved_bugs, $iRows, $iColumns)

;	$query = "INSERT INTO BugTotalResolvedPerDate (Date,TotalBugs,TotalBugsResolved) VALUES ('" & $sqlite_date & "','" & $num_opened_bugs[1][0] & "','" & $num_resolved_bugs[1][0] & "');"
;	_FileWriteLog($log_filepath, "BugTotalResolvedPerDate for date " & $sqlite_date & " = " & $query)
...
;			GUICtrlSetData($status_input, "Creating and Posting Full Traceability report ...")
;			Create_HTML_Report(True)
;			Local $html = FileRead(@ScriptDir & "\html_report.html")
;			Update_Confluence_Page("https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "386105349", "RMS Full Traceability Report", $html)

			Create_And_Post_Reports_For_Version("drop", "all", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "387285777", "RMS All Drops Reports")
			Create_And_Post_Reports_For_Version("drop", "RMS - Drop 1", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "387253477", "RMS Drop 1 Reports")
			Create_And_Post_Reports_For_Version("drop", "#0.1.3.15 - RMS - Drop 1.1", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "387055788", "RMS Drop 1.1 Reports")
			Create_And_Post_Reports_For_Version("drop", "RMS - Drop 1.2", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "387219484", "RMS Drop 1.2 Reports")
			Create_And_Post_Reports_For_Version("drop", "RMS - Drop 2", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "386728182", "RMS Drop 2 Reports")
			Create_And_Post_Reports_For_Version("drop", "RMS - Drop 3", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "386826341", "389906800", "RMS Drop 3 Reports")

			; ITE

;			Create_And_Post_Reports_For_Version("release", "all", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "387973774", "ITE All Releases Reports")
;			Create_And_Post_Reports_For_Version("release", "UAT Release 1", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "388203018", "ITE UAT Release 1 Reports")
;			Create_And_Post_Reports_For_Version("release", "UAT Release 1.1", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "388006215", "ITE UAT Release 1.1 Reports")
;			Create_And_Post_Reports_For_Version("release", "UAT Release 1.3", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "388006237", "ITE UAT Release 1.3 Reports")
;			Create_And_Post_Reports_For_Version("release", "UAT 3.4 HOTFIX 12", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "388170210", "ITE UAT 3.4 HOTFIX 12 Reports")
;			Create_And_Post_Reports_For_Version("release", "Release 4", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "388203035", "ITE Release 4 Reports")
;			Create_And_Post_Reports_For_Version("release", "Release 4.1", "https://janisoncls.atlassian.net", GUICtrlRead($jira_username_input), GUICtrlRead($jira_password_input), "JAST", "388006194", "387973822", "ITE Release 4.1 Reports")
...
;						$a = StringRegExpReplace($a, "([^\\])\\$", "$1\\\\")
	...
	$storage_format = $storage_format &	"</ac:structured-macro>" & @CRLF
...
Func Create_Defect_Opened_Report($affects_version)

	_SQLite_Open(@ScriptDir & "\" & $app_name & ".sqlite")
;	$storage_format	= ""
;	SQLite_to_Confluence_Chart("timeSeries", "Defect Priorities for the last 6 months", "", "Date", "Total", "true", "false", "1024", "600", "false", "", "", "", "1,2,3", "vertical", "", "", "day", "", "", "true", "", "", "select Date, TotalBugs, TotalBugsResolved from BugTotalResolvedPerDate;")
	SQLite_to_Confluence_Chart("area", "Defects Opened Weekly, during the latest 12 Months, by Priority", "", "Week Starting", "Number of Defects", "true", "true", "1024", "600", "false", "", "", "", "1,2,3,4,5,6", "vertical", "", "", "day", "", "", "true", $trivial_colour & "," & $minor_colour & "," & $major_colour & "," & $critical_colour & "," & $blocker_colour, "", $max_num_defects, "up45", "select WeekStarting, TrivialOpened as ""Trivial"", MinorOpened as ""Minor"", MajorOpened as ""Major"", CriticalOpened as ""Critical"", BlockersOpened as ""Blocker"" from BugOpenedPerWeek where AffectsVersion = '" & $affects_version & "';")
	FileDelete(@ScriptDir & "\defect_opened_report.sf")
	FileWrite(@ScriptDir & "\defect_opened_report.sf", $storage_format)

EndFunc

Func Create_Defect_Priority_Report($affects_version)

	_SQLite_Open(@ScriptDir & "\" & $app_name & ".sqlite")
;	$storage_format	= ""
;	SQLite_to_Confluence_Chart("timeSeries", "Defect Priorities for the last 6 months", "", "Date", "Total", "true", "false", "1024", "600", "false", "", "", "", "1,2,3", "vertical", "", "", "day", "", "", "true", "", "", "select Date, TotalBugs, TotalBugsResolved from BugTotalResolvedPerDate;")
	SQLite_to_Confluence_Chart("area", "Total Defects by Priority", "", "Date", "Number of Defects", "true", "true", "1024", "600", "false", "", "", "", "1,2,3,4,5,6", "vertical", "", "", "day", "", "", "true", $trivial_colour & "," & $minor_colour & "," & $major_colour & "," & $critical_colour & "," & $blocker_colour, "", $max_num_defects, "up45", "select Date, TrivialOpened as ""Trivial"", MinorOpened as ""Minor"", MajorOpened as ""Major"", CriticalOpened as ""Critical"", BlockersOpened as ""Blocker"" from BugTotalOpenedPerDate where AffectsVersion = '" & $affects_version & "';")
	FileDelete(@ScriptDir & "\defect_priorities_report.sf")
	FileWrite(@ScriptDir & "\defect_priorities_report.sf", $storage_format)

EndFunc

Func Create_Defects_Resolved_Report($affects_version)

	_SQLite_Open(@ScriptDir & "\" & $app_name & ".sqlite")
;	$storage_format	= ""
;	SQLite_to_Confluence_Chart("timeSeries", "Defects Resolved, in Total, for the last 6 months (Defect Removal Efficiency)", "", "Date", "Total", "true", "false", "1024", "600", "false", "", "", "", "1,2,3", "vertical", "", "", "day", "", "", "true", "", "", "select Date, TotalBugs as ""Total Opened"", TotalBugsResolved as ""Total Resolved"" from BugTotalResolvedPerDate;")
	SQLite_to_Confluence_Chart("line", "Total Resolved Defects (Defect Removal Efficiency)", "", "Date", "Number of Defects", "true", "false", "1024", "600", "false", "", "", "", "1,2,3", "vertical", "", "", "day", "", "", "true", "#DCDCDC,#000000", "", $max_num_defects, "up45", "select Date, TotalBugs as ""Total Created"", TotalBugsResolved as ""Total Resolved"" from BugTotalResolvedPerDate where AffectsVersion = '" & $affects_version & "';")
	SQLite_to_Confluence_Chart("area", "Total Resolved Defects by Priority", "", "Date", "Number of Defects Resolved", "true", "true", "1024", "600", "false", "", "", "", "1,2,3,4,5,6", "vertical", "", "", "day", "", "", "true", $unresolved_colour & "," & $trivial_colour & "," & $minor_colour & "," & $major_colour & "," & $critical_colour & "," & $blocker_colour, "", $max_num_defects, "up45", "select Date, TotalTrivialResolved as ""Trivial"", TotalMinorResolved as ""Minor"", TotalMajorResolved as ""Major"", TotalCriticalResolved as ""Critical"", TotalBlockersResolved as ""Blocker"" from BugTotalResolvedPerDate where AffectsVersion = '" & $affects_version & "';")
	FileDelete(@ScriptDir & "\defect_efficiency_removal_report.sf")
	FileWrite(@ScriptDir & "\defect_efficiency_removal_report.sf", $storage_format)

EndFunc

Func Create_Defects_Unresolved_Report($affects_version)

	_SQLite_Open(@ScriptDir & "\" & $app_name & ".sqlite")
;	$storage_format	= ""
;	SQLite_to_Confluence_Chart("timeSeries", "Number of Unresolved Defects for the last 6 months", "", "Defects Created in Week Starting", "Total Defects", "false", "false", "1024", "600", "false", "", "", "", "1,2", "vertical", "", "", "day", "", "", "true", "", "up45", "select Date, TotalActive from BugTotalActivePerDate;")
	SQLite_to_Confluence_Chart("line", "Total Unresolved Defects", "", "Date", "Number of Defects", "true", "false", "1024", "600", "false", "", "", "", "1,2,3", "vertical", "", "", "day", "", "", "true", "#DCDCDC,#000000", "", $max_num_defects, "up45", "select BugTotalActivePerDate.Date, BugTotalResolvedPerDate.TotalBugs as ""Total Created"", BugTotalActivePerDate.TotalActive as ""Total Unresolved"" from BugTotalActivePerDate, BugTotalResolvedPerDate where BugTotalActivePerDate.AffectsVersion = BugTotalResolvedPerDate.AffectsVersion and BugTotalActivePerDate.Date = BugTotalResolvedPerDate.Date and BugTotalActivePerDate.AffectsVersion = '" & $affects_version & "';")
	SQLite_to_Confluence_Chart("area", "Total Unresolved Defects by Priority", "", "Date", "Number of Defects Unresolved", "true", "true", "1024", "600", "false", "", "", "", "1,2,3,4,5,6", "vertical", "", "", "day", "", "", "true", $trivial_colour & "," & $minor_colour & "," & $major_colour & "," & $critical_colour & "," & $blocker_colour, "", $max_num_defects, "up45", "select Date, TrivialActive as ""Trivial"", MinorActive as ""Minor"", MajorActive as ""Major"", CriticalActive as ""Critical"", BlockersActive as ""Blocker"" from BugTotalActivePerDate where AffectsVersion = '" & $affects_version & "';")
	FileDelete(@ScriptDir & "\defects_active_report.sf")
	FileWrite(@ScriptDir & "\defects_active_report.sf", $storage_format)

EndFunc

Func Create_Defect_Unresolved_Age_Report($affects_version)

	_SQLite_Open(@ScriptDir & "\" & $app_name & ".sqlite")
;	$storage_format	= ""
	_SQLite_GetTable2d(-1, "select max(max(a.BlockerUnresolvedAge), max(b.CriticalUnresolvedAge), max(c.MajorUnresolvedAge)) from BugStateDate a, BugStateDate b, BugStateDate c where a.BlockerUnresolvedAge != '' and a.AffectsVersion = '" & $affects_version & "' and b.CriticalUnresolvedAge != '' and b.AffectsVersion = '" & $affects_version & "' and c.MajorUnresolvedAge != '' and c.AffectsVersion = '" & $affects_version & "';", $aResult, $iRows, $iColumns)
	$max_num_days = $aResult[1][0]
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $max_num_days = ' & $max_num_days & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;	SQLite_to_Confluence_Chart("bar", "Unresolved Defect Age, per Priority", "", "Date", "Total", "true", "false", "1024", "600", "false", "", "", "", "1,2,3,4,5,6", "vertical", "", "", "day", "", "", "true", "", "", "select Key, BlockerUnresolvedAge, CriticalUnresolvedAge, MajorUnresolvedAge, MinorUnresolvedAge, TrivialUnresolvedAge from BugStateDate;")
	SQLite_to_Confluence_Chart("bar", "Unresolved Age of all Blocker Defects", "", "Blocker Defect ID", "Number of Days Unresolved", "false", "false", "1024", "600", "false", "", "", "", "1,2", "vertical", "", "", "day", "", "", "true", $blocker_colour, "", $max_num_days, "up45", "select Key, BlockerUnresolvedAge as ""Blocker"" from BugStateDate where AffectsVersion = '" & $affects_version & "' and Priority = 'Blocker' order by BlockerUnresolvedAge;")
	SQLite_to_Confluence_Chart("bar", "Unresolved Age of all Critical Defects", "", "Critical Defect ID", "Number of Days Unresolved", "false", "false", "1024", "600", "false", "", "", "", "1,2", "vertical", "", "", "day", "", "", "true", $critical_colour, "", $max_num_days, "up90", "select Key, CriticalUnresolvedAge as ""Critical"" from BugStateDate where AffectsVersion = '" & $affects_version & "' and Priority = 'Critical' order by CriticalUnresolvedAge;")
	SQLite_to_Confluence_Chart("bar", "Unresolved Age of all Major Defects", "", "Major Defect ID", "Number of Days Unresolved", "false", "false", "1024", "600", "false", "", "", "", "1,2", "vertical", "", "", "day", "", "", "true", $major_colour, "", $max_num_days, "up90", "select Key, MajorUnresolvedAge as ""Major"" from BugStateDate where AffectsVersion = '" & $affects_version & "' and Priority = 'Major' order by MajorUnresolvedAge;")
	FileDelete(@ScriptDir & "\unresolved_defect_age_report.sf")
	FileWrite(@ScriptDir & "\unresolved_defect_age_report.sf", $storage_format)
...
Func Create_And_Post_Reports_For_Version($version_type, $version_name, $url, $jira_username, $jira_password, $space_key, $ancestor_key, $page_key, $page_title)

	GUICtrlSetData($status_input, "Creating and Posting reports for " & $version_type & " " & $version_name & " ...")
	$storage_format	= ""
	_SQLite_GetTable2d(-1, "select max(TotalBugs) from BugTotalResolvedPerDate where AffectsVersion = '" & $version_name & "';", $aResult, $iRows, $iColumns)
	$max_num_defects = $aResult[1][0]
	Create_Defect_Opened_Report($version_name)
	Create_Defect_Priority_Report($version_name)
	Create_Defects_Resolved_Report($version_name)
	Create_Defects_Unresolved_Report($version_name)
	Create_Defect_Unresolved_Age_Report($version_name)
	Update_Confluence_Page($url, $jira_username, $jira_password, $space_key, $ancestor_key, $page_key, $page_title, $storage_format)
	GUICtrlSetData($status_input, "")
EndFunc
